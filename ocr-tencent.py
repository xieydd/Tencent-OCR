...
class MsgTencent(object):
    ...
    def __get_image_base64str__(self,image):
        ...
        BytesIO.bg.save(outputBuffer, format='JPEG')
    ...
    def get_img_base64str(self,image):
        ...
        elif isinstance(image,type(Image)):
            self.img_base64str= self.__get_imgfile_base64str__(image)
...
def ExecTecentAPI(*arg,**kwds):
    ...
    print (name+'执行结果'+resp.text.encode("utf-8"))
    return resp.json()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(img_path + "json"):
        os.mkdir(img_path + "json")
    for file in glob.glob(img_path + '*.jpg'):
        #手写体ocr
        #file= r'/Users/xieyuandong/Downloads/1.jpg'
        fsize = os.path.getsize(file)
        fsize = fsize/float(1024*1024)
        if fsize >= 1:
            sImg=Image.open(file)
            w,h=sImg.size
            value=sImg.resize((w*9/10,h*9/10),Image.ANTIALIAS)
            value.save(file)
        time.sleep(0.55)
        rest = ExecTecentAPI(Apiname='ocr_handwritingocr',image=file)

        
        #print(rest['data']['item_list'])
        jsonpath = img_path + "json/" + file.split('/')[-1].split('.')[0]
        if not os.path.exists(jsonpath):
            os.mkdir(jsonpath)
        
        j = 1
        for i in rest['data']['item_list']:
            jsonfile = img_path + "json/" + file.split('/')[-1].split('.')[0] + "/" + file.split('/')[-1].split('.')[0] + "-"  + str(j) +".json"
            j = j+1
            if os.path.exists(jsonfile):
                continue
            with open(jsonfile,"a") as f:
                json.dump(i['itemcoord'][0],f,indent=4)
    print("加载入文件完成...")
